...
export class BoloTieBehavior3D implements AIBehavior3D {
  ...
  update(self: Ship3D, target: Ship3D, dt: number, now: number): ShipInput & { fire: boolean } {
    ...
    switch (this.phase) {
      ...
      case 'engage': {
        ...
        if (spd > 150 * self.speedMult) self.velocity.setLength(150 * self.speedMult);
  ...
  private _setPhase(phase: Phase): void {
    ...
    switch (phase) {
      ...
      case 'engage':    this.phaseDuration = 1.5 + r * 1.5; break;
      case 'overshoot': this.phaseDuration = 0.8 + r * 0.5; break;
      case 'evade': {
        this.phaseDuration = 2.5 + r * 1.5;
        this.maneuverDir *= -1;
