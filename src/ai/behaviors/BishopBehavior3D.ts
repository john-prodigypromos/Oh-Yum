...
// Sharp dogfighter + HP-gated escalation + drone spawn.
...
export class BishopBehavior3D implements AIBehavior3D {
  ...
  update(self: Ship3D, target: Ship3D, dt: number, now: number): ShipInput & { fire: boolean } {
    ...
    // Effective tuning escalates with boss phase
    ...
    switch (this.phase) {
      ...
      case 'evade': {
        const steer = steerAway(self, target.position, phaseSens * 1.5, 0.5, this.breakDir * 1.4);
        yaw = steer.yaw;
    ...
    const desperationJink = this.bossPhase === 'phase3' ? 1.2
      : this.bossPhase === 'phase2' ? 0.8 : 0.5;
    const jinkBase = this.phase === 'evade' ? 1.0 : (0.2 + aggression * 0.4);
    const jink = jinkOverlay(this.timer, this.seed, this.cfg.jinkIntensity * jinkBase * desperationJink);
  ...
  private _setPhase(phase: Phase): void {
    ...
    const berserk = this.bossPhase === 'phase3' ? 0.4 : 1;
    const r = (chaos(this.timer, this.seed) + 1) * 0.5;
    switch (phase) {
      case 'chase':     this.phaseDuration = 5; break;
      case 'engage':    this.phaseDuration = (1.5 + r * 1.5) * aggrScale * berserk; break;
      case 'overshoot': this.phaseDuration = (0.15 + r * 0.2) * berserk; break;
      case 'evade':     this.phaseDuration = (0.2 + r * 0.3) * aggrScale * berserk; break;
    }
  }
}
